...
The model emits a single JSON object (see gemini-review.yml prompt):

    {"summary": "<markdown>", "comments": [{"path","line","severity","body"}, ...]}

We anchor each comment to a real diff line and post ONE review (event=COMMENT)
with the summary as its body + the findings as inline comments. Findings whose
(path, line) can't be matched to the diff are appended to the summary so nothing
is lost. Reads: env REVIEW_JSON, PR, REPO, and the file pr.diff. stdlib only.
"""
...
def _diff_right_lines(path: str) -> dict[str, set[int]]:
    """Map file -> set of NEW-file line numbers that appear in the diff (added or
    context lines on the RIGHT side); those are the lines GitHub lets us comment on."""
        ...
        for line in f:
            if line.startswith("+++ "):
                ...
                if line.startswith(("+", " ")):
                    ...
                    new_ln += 1
                # '-' lines are RIGHT-side absent; '\ No newline' etc. are ignored
    return valid
...
def main() -> int:
    ...
    for c in comments:
        ...
        if not body:
            continue
        tag = f"**{c['severity']}**: " if c.get("severity") else ""
        if p in valid and isinstance(ln, int) and ln in valid[p]:
            inline.append({"path": p, "line": ln, "side": "RIGHT", "body": tag + body})
        else:
            orphan.append(f"- `{p}:{ln}` — {tag}{body}")
